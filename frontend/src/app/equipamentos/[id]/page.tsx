...
import AuditoriaTab from "@/components/domain/AuditoriaTab";
...
function calcDesgaste(h: number, v: number) {
  return v > 0 ? (h / v) * 100 : 0;
}
function desgasteColor(p: number) {
  return p >= 100
    ? "var(--red-badge)"
    : p >= 85
    ? "var(--yellow-badge)"
...
function desgasteLabel(p: number) {
  return p >= 100 ? "Crítico" : p >= 85 ? "Atenção" : "Saudável";
}
...
export default function EquipamentoDetalhesPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "VISAO_GERAL" | "DIAGNOSTICOS" | "MODIFICACOES" | "HISTORICO" | "AUDITORIA"
  >("VISAO_GERAL");
  ...
  return (
    ...
    >
      ...
      {/* Tabs */}
      <div
        className="flex items-center gap-8 border-b mb-8"
        style={{ borderColor: "var(--border-subtle)" }}
      >
        <button
          onClick={() => setActiveTab("VISAO_GERAL")}
          className={`pb-4 text-[14px] font-semibold transition-all cursor-pointer relative ${
            activeTab === "VISAO_GERAL" ? "text-txt-primary" : "text-txt-muted hover:text-txt-secondary"
          }`}
        >
          Visão Geral & Componentes
          {activeTab === "VISAO_GERAL" && (
            <div className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ background: "var(--teal)" }} />
          )}
        </button>
        <button
          onClick={() => setActiveTab("DIAGNOSTICOS")}
          className={`pb-4 text-[14px] font-semibold transition-all cursor-pointer relative ${
            activeTab === "DIAGNOSTICOS" ? "text-txt-primary" : "text-txt-muted hover:text-txt-secondary"
          }`}
        >
          Diagnóstico
        ...
        <button
          ...
          className={`pb-4 text-[14px] font-semibold transition-all cursor-pointer relative ${
            activeTab === "MODIFICACOES" ? "text-txt-primary" : "text-txt-muted hover:text-txt-secondary"
          }`}
        >
          Modificações & BOM
        ...
        <button
          ...
          className={`pb-4 text-[14px] font-semibold transition-all cursor-pointer relative ${
            activeTab === "HISTORICO" ? "text-txt-primary" : "text-txt-muted hover:text-txt-secondary"
          }`}
        >
          Histórico
          {activeTab === "HISTORICO" && (
        ...
        <button
          ...
          className={`pb-4 text-[14px] font-semibold transition-all cursor-pointer relative ${
            activeTab === "AUDITORIA" ? "text-txt-primary" : "text-txt-muted hover:text-txt-secondary"
          }`}
        >
          Auditoria
          {activeTab === "AUDITORIA" && (
      ...
      {activeTab === "VISAO_GERAL" ? (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
            {[
              ...
              <div
                key={item.label}
                className="glass-card p-space-lg"
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2">
                          <h4 className="text-[16px] font-semibold truncate text-txt-primary">
                            {comp.nome}
                          </h4>
                          <span
                            className="px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider shrink-0"
                            style={{ background: bgVal, color: cor }}
                          >
                            {label}
                          </span>
                          {comp.modificado && (
                            <span
                              className="flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider shrink-0"
                              style={{
                                background: "var(--cyan-badge-bg)",
                                color: "var(--cyan-badge)",
                                border: "1px solid var(--cyan-badge-border)",
                            ...
                            >
                              ...
                              >
                                <path
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
                                  d="M9.813 15.904 9 21l8.982-11.795H14l1-7L6 14.09h3.813Z"
                                />
                              </svg>
                              Modificado
                            </span>
                      ...
                      <div className="text-right shrink-0 mr-6">
                        ...
                        <p className="text-[16px] font-bold text-txt-primary">
                          {comp.horasOperacionais.toLocaleString("pt-BR")}{" "}
                          <span className="text-[11px] font-normal text-txt-muted">
                            / {comp.vidaUtilNominal.toLocaleString("pt-BR")} h
                          </span>
                        </p>
                      </div>
                      ...
                      <div className="flex items-center gap-6 shrink-0">
                        <div className="w-36">
                          ...
                          >
                            <div
                              ...
                              style={{ width: `${Math.min(desgaste, 100)}%`, background: cor }}
                            />
                          </div>
                        </div>
                        ...
                        >
                          <button
                            onClick={() => setReporteModalComp({ ...comp, desgastePct: desgaste })}
                            ...
                            onMouseEnter={(e) => {
                              e.currentTarget.style.background = "rgba(232, 132, 44, 0.18)";
                            }}
                            onMouseLeave={(e) => {
                        ...
                        {canWrite && (
                          ...
                          >
                            <button
                              onClick={() => {
                                setEditComp(comp as ComponenteComDesgaste);
                                setCompModalOpen(true);
                              }}
        ...
        <AuditoriaTab equipamentoId={equipamento.id} />
      )}
        ...
        >
          ...
          >
            ...
            <p className="text-[14px] mb-6 text-txt-secondary">
              Tem certeza que deseja remover este componente? O histórico de horas e desgaste será
              perdido.
            </p>
            <div className="flex justify-end gap-3">
              <button
                onClick={() => setDeleteCompId(null)}
