...
export default function EquipamentoDetalhesPage() {
  ...
  if (erro || !equipamento) {
    return (
      <div className="p-8">
        <div className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm" style={{ background: "var(--red-badge-bg)", border: "1px solid var(--red-badge-border)", color: "var(--red-badge)" }}>
          {erro || "Equipamento não encontrado."}
        </div>
        <button onClick={() => router.push("/equipamentos")} className="mt-4 text-[14px] cursor-pointer" style={{ color: "var(--orange)" }}>
  ...
  return (
    <div className="page-container animate-fade-in-up" style={{ animationFillMode: "both" }}>
      ...
      <div className="flex items-center gap-2 mb-8 text-[13px]">
        <button onClick={() => router.push("/equipamentos")} className="transition-colors duration-200 cursor-pointer hover:underline" style={{ color: "var(--text-muted)" }}
          onMouseEnter={e => { e.currentTarget.style.color = "var(--orange)"; }}
          onMouseLeave={e => { e.currentTarget.style.color = "var(--text-muted)"; }}>
          Equipamentos
        </button>
        <span style={{ color: "var(--border-subtle)" }}>/</span>
        <span style={{ color: "var(--text-secondary)" }}>{equipamento.tag}</span>
      </div>
      ...
      <div className="flex items-start justify-between mb-10">
        <div>
          ...
          <p className="text-[16px] text-txt-muted">{equipamento.tipo}</p>
        </div>
      ...
      {/* Tabs */}
      <div className="flex items-center gap-8 border-b mb-8" style={{ borderColor: "var(--border-subtle)" }}>
        <button
          onClick={() => setActiveTab("VISAO_GERAL")}
          className={`pb-4 text-[14px] font-semibold transition-all cursor-pointer relative ${activeTab === "VISAO_GERAL" ? "text-txt-primary" : "text-txt-muted hover:text-txt-secondary"}`}
        >
          Visão Geral & Componentes
          {activeTab === "VISAO_GERAL" && <div className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ background: "var(--teal)" }} />}
        </button>
        <button
          onClick={() => setActiveTab("DIAGNOSTICOS")}
          className={`pb-4 text-[14px] font-semibold transition-all cursor-pointer relative ${activeTab === "DIAGNOSTICOS" ? "text-txt-primary" : "text-txt-muted hover:text-txt-secondary"}`}
        >
          Diagnóstico
          {activeTab === "DIAGNOSTICOS" && <div className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ background: "var(--teal)" }} />}
        </button>
        <button
          onClick={() => setActiveTab("MODIFICACOES")}
          className={`pb-4 text-[14px] font-semibold transition-all cursor-pointer relative ${activeTab === "MODIFICACOES" ? "text-txt-primary" : "text-txt-muted hover:text-txt-secondary"}`}
        >
          Modificações & BOM
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
            {[
              ...
              <div key={item.label} className="glass-card p-space-lg" style={{ borderRadius: "14px" }}>
                <p className="text-[10px] uppercase tracking-widest mb-2 text-txt-muted">{item.label}</p>
                <p className="text-[16px] font-semibold text-txt-primary">{item.value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-10">
            <div className="glass-card p-space-lg" style={{ borderRadius: "14px" }}>
              <p className="text-[10px] uppercase tracking-widest mb-2 text-txt-muted">Localização</p>
              <p className="text-[16px] font-medium text-txt-primary">{equipamento.localizacao}</p>
            </div>
            {equipamento.descricao && (
              <div className="glass-card p-space-lg" style={{ borderRadius: "14px" }}>
                <p className="text-[10px] uppercase tracking-widest mb-2 text-txt-muted">Descrição</p>
                <p className="text-[14px] leading-relaxed text-txt-secondary">{equipamento.descricao}</p>
              </div>
            )}
          </div>
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-7">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-[22px] font-bold text-txt-primary">Componentes Mecânicos</h2>
                <span className="px-2.5 py-0.5 rounded-full text-[11px] font-semibold" style={{ background: "var(--cyan-badge-bg)", color: "var(--cyan-badge)" }}>
              ...
              {canWrite && (
                <button
                  ...
                  style={{ background: "var(--teal)", boxShadow: "0 4px 12px var(--orange-glow)" }}
            ...
            {equipamento.componentes.length === 0 ? (
              <div className="glass-card flex flex-col items-center justify-center py-12 text-center" style={{ borderRadius: "12px" }}>
                <svg className="w-12 h-12 mb-3" fill="none" viewBox="0 0 24 24" stroke="var(--text-muted)" strokeWidth={0.8}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z" />
                </svg>
                <p className="text-[15px] font-medium text-txt-secondary">Nenhum componente cadastrado</p>
              </div>
            ) : (
              <div className="space-y-4">
                {equipamento.componentes.map((comp, i) => {
                  ...
                  return (
                    <div
                      key={comp.id}
                      className="glass-card p-space-lg flex items-center gap-6 animate-fade-in-up"
                      style={{ borderRadius: "12px", animationDelay: `${i * 0.08}s`, animationFillMode: "both", borderLeft: `3px solid ${cor}` }}
                    >
                      {/* Icon */}
                      <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0" style={{ background: bgVal }}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2">
                          <h4 className="text-[16px] font-semibold truncate text-txt-primary">{comp.nome}</h4>
                          <span className="px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider shrink-0" style={{ background: bgVal, color: cor }}>
                            {label}
                          </span>
                          {comp.modificado && (
                            <span className="flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider shrink-0" style={{ background: "var(--cyan-badge-bg)", color: "var(--cyan-badge)", border: "1px solid var(--cyan-badge-border)" }}>
                        ...
                        <p className="text-[13px] mt-1 text-txt-muted">
                          Tipo: <span className="text-txt-secondary">{comp.tipo.replace(/_/g, " ")}</span>
                        </p>
                      </div>

                      {/* Hours info */}
                      <div className="text-right shrink-0 mr-6">
                        <p className="text-[12px] mb-1 text-txt-muted">Horas Operacionais</p>
                        <p className="text-[16px] font-bold text-txt-primary">
                          {comp.horasOperacionais.toLocaleString("pt-BR")} <span className="text-[11px] font-normal text-txt-muted">/ {comp.vidaUtilNominal.toLocaleString("pt-BR")} h</span>
                        </p>
                      </div>
                      ...
                      <div className="flex items-center gap-6 shrink-0">
                        ...
                        {canWrite && (
                          <div className="flex items-center gap-2 border-l pl-6" style={{ borderColor: "var(--border-subtle)" }}>
                            <button
      ...
      {deleteCompId && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }}>
          <div className="glass-card p-8 max-w-md w-full mx-4" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-subtle)" }}>
            <div className="flex items-center gap-3 mb-4">
